...
<template>
  <div class="flex h-96 w-1/5 flex-col items-center rounded-xl border p-2">
    <span>{{ index + 1 }}</span>
    ...
    <div class="justify-content my-auto grid grid-cols-2 gap-2">
      <div
        v-for="song in artist.songs.slice(0, 4)"
        :key="song.id"
        class="max-w-1/2 flex justify-center gap-1 rounded-xl border p-1"
      >
